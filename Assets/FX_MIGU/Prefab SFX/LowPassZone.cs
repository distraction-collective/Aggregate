...
public class LowPassZone : MonoBehaviour
{
    ...
    public string lowPassParameterName = "LowPassExT"; // Parameter name for LowPass filter
    ...
    void Start()
    {
        ...
        // Initialize Audio Mixer parameters
        if (audioMixer != null)
        {
            audioMixer.SetFloat(lowPassParameterName, 22000f); // Start with normal lowpass frequency (22000 Hz)
        }
    ...
    void CheckDistanceToObject()
    {
        if (soundObject != null && playerTransform != null)
        {
            ...
            // Update LowPass based on distance
    ...
    private void UpdateLowPassFilter(float distance)
    {
        // Update LowPass frequency based on distance
        if (audioMixer != null)
        {
            float lowPassValue;
            ...
            if (distance >= maxDistance)
            {
                lowPassValue = 22000f; // Full frequency if outside max distance
            }
            else
            {
                // Calculate the lowpass frequency
                float normalizedDistance = Mathf.Clamp01((distance - minDistance) / (maxDistance - minDistance));
                lowPassValue = Mathf.Lerp(400f, 22000f, Mathf.Pow(normalizedDistance, volumeExponent));
            }

            audioMixer.SetFloat(lowPassParameterName, lowPassValue);
        }
    }
