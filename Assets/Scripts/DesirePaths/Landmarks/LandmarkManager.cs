...
namespace DesirePaths.Landmarks {
  public class LandmarkManager : MonoBehaviour {
    ...
    private string _playerTag =
        "Player_Collider"; // the collider used is under puppet master / pelvis

    private void Awake() {
      _landmarks.ForEach(x => {
        x.SetPlayerTag(_playerTag);
        if (x.GetType() == typeof(Pillar))
          _pillarCount += 1;
        _landmarkCount += 1;
      });
    ...
    private void OnDisable() { SubscribeToLandmarkTriggers(false); }
