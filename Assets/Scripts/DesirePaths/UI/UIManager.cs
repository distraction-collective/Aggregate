...
namespace DesirePaths.UI {
  public class UIManager : MonoBehaviour {
    [SerializeField]
    private SubtitlesDisplayer _subtitles;
    [SerializeField]
    private CanvasGroup _fadeElement;

#region SUBS
    public void DisplaySubs(string subText) { _subtitles.Set(subText); }
#endregion

#region TRANSITION UI EFFECTS

    public void HideNShow() { StartCoroutine(HideAndShowFade(2f, 0.5f)); }
    ...
    private IEnumerator HideAndShowFade(float duration, float hold) {
      ...
      while (_currentDuration < _halfDuration) {
        ...
        _fadeElement.alpha =
            Mathf.Lerp(0f, 1f, _currentDuration / _halfDuration);
        yield return null;
      }
      ...
      while (_currentDuration < _halfDuration) {
        _currentDuration += Time.deltaTime;
        _fadeElement.alpha =
            Mathf.Lerp(1f, 0f, _currentDuration / _halfDuration);
        yield return null;
      }
