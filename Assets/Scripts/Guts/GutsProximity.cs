...
public class GutsProximity : MonoBehaviour {
  ...
  public bool GetSafe() { return isAttached; }

  // Update is called once per frame
  void Update() { DetectionTest(); }

  public bool GetAttached() { return isAttached; }

  private void LateUpdate() { PlaceOnPosition(); }
  ...
  private void DetectionTest() {
    SplineUtility.GetNearestPoint<Spline>(
        chosenSpline, playerTransform.position, out nearestPoint,
        out nearestInterpolation);
    worldSpaceNearestPoint = (Vector3)nearestPoint;

    isAttached = Vector3.Distance(playerTransform.position,
                                  worldSpaceNearestPoint) <= detectRange
                     ? true
                     : false;
  }
  ...
  private void PlaceOnPosition() {
    if (isAttached) {
      ...
      var newTangent = CadaverGutsManager.Vector3ToFloat3(
          ((playerTransform.position - followerTransform.position).normalized -
           Vector3.up)
              .normalized);
      var newTangent2 = CadaverGutsManager.Vector3ToFloat3(
          ((playerTransform.position - followerTransform.position).normalized +
           Vector3.up)
              .normalized);
      _splineContainer.Spline.Add(
...
#if UNITY_EDITOR
      Debug.DrawRay(playerTransform.position,
                    worldSpaceNearestPoint - playerTransform.position,
                    Color.green);
      // Debug.Log("nearestPoint: " + nearestPoint + " " +
      // (worldSpaceNearestPoint - playerTransform.position).magnitude + " " +
