...
#region Functions
  ...
  void UpdateCamera(ScriptableRenderContext SRC, Camera camera) {
    if ((camera.cameraType == CameraType.Game ||
        ...
        camera.tag !=
            "PortalCam") // is the current camera eligeble for portalling?
    {
      ...
      if (!enabled || !rend || !rend.sharedMaterial ||
          !rend.enabled) // <<<< Why does the renderer NEED to have a shared
                         // material??
        return;
      ...
      // Render the camera
      RenderCamera(camera, rend, Camera.StereoscopicEye.Left,
                   ref m_PortalTextureLeft, SRC);
      if (camera.stereoEnabled) {
        try {
          RenderCamera(camera, rend, Camera.StereoscopicEye.Right,
                       ref m_PortalTextureRight, SRC);
        } catch (Exception e) {
  ...
  private void RenderCamera(Camera camera, Renderer rend,
    ...
    CopyCameraProperties(
        camera, portalCamera); // Copy the properties of the (player) camera
    ...
    Vector3 normal = transform.TransformDirection(
        projectionDirection); // Alex: This is done because sometimes the object
                              // reflection direction does not align with what
                              // was the default (transform.forward), in this
                              // way, the user can specify this.
    // normal.Normalize(); // Alex: normalize in case someone enters a
    // non-normalized vector. Turned off for now because it is a fun effect :P
    ...
    if (camera.stereoEnabled) {
      ...
      if (eye == Camera.StereoscopicEye.Left)
        leftEyeState.TryGetPosition(out eyeOffset); // eyeOffset =
                                                    // InputTracking.GetLocalPosition(XRNode.LeftEye);
                                                    // //<< Deprecated
      else
        rightEyeState.TryGetPosition(out eyeOffset); // eyeOffset =
                                                     // InputTracking.GetLocalPosition(XRNode.RightEye);
                                                     // //<< Deprecated

      eyeOffset.z = 0.0f;
      oldEyePos = camera.transform.position +
                  camera.transform.TransformVector(eyeOffset);
    } else {
    ...
    portalCamera.projectionMatrix =
        camera.projectionMatrix; // Match matrices <<<
    ...
    Vector4 clipPlane =
        CameraSpacePlane(worldToCameraMatrix * reflection, pos, normal, 1.0f);
    ...
    UniversalRenderPipeline.RenderSingleCamera(
        SRC, portalCamera); // URP Version of: portalCamera.Render();

    GL.invertCulling = false;

    // Assign the rendertexture to the material
    Material[] materials =
        rend.sharedMaterials; // Why only get the shared materials?
  ...
  private void CreatePortalCamera(Camera currentCamera,
                                  Camera.StereoscopicEye eye,
    ...
    if (!portalTexture ||
        m_OldReflectionTextureSize !=
            m_TextureSize) // if it doesn't exist or the size has changed
    {
      if (portalTexture)                 // if it does exist
        DestroyImmediate(portalTexture); // destroy it first

      portalTexture = new RenderTexture(m_TextureSize, m_TextureSize,
                                        24); // <<<< make buffer size 24??
      portalTexture.name = "__MirrorReflection" + eye.ToString() +
                           GetInstanceID(); // create the name of the object
      portalTexture.isPowerOfTwo =
          true; // https://docs.unity3d.com/Manual/Textures.html: Non power of
                // two texture assets can be scaled up at import time using the
                // Non Power of 2 option in the advanced texture type in the
                // import settings. Unity will scale texture contents as
                // requested, and in the game they will behave just like any
                // other texture, so they can still be compressed and very fast
                // to load.
      portalTexture.hideFlags =
          HideFlags
              .DontSave; // The object will not be saved to the Scene. It will
                         // not be destroyed when a new Scene is loaded.

      portalTexture.antiAliasing =
          4; // < <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ResourceIntensive but
             // pretty
    ...
    if (!m_PortalCameras.TryGetValue(
            currentCamera,
            out portalCamera)) // if it does not yet exist in the dictionary,
                               // create it. If it does, assign it. (catch both
                               // not-in-dictionary and
                               // in-dictionary-but-deleted-GO)
    {
      GameObject go = new GameObject(
          "Mirror Reflection Camera id" + GetInstanceID() + " for " +
      ...
      portalCamera.tag =
          "PortalCam"; // Tag it as a portal camera so it doesn't participate in
                       // the additional CameraRender function
      // portalCamera.gameObject.AddComponent<FlareLayer>(); // Adds a flare
      // layer to make Lens Flares appear in the image?? disabled for now
      go.hideFlags =
          HideFlags
              .DontSave; // The object will not be saved to the Scene. It will
                         // not be destroyed when a new Scene is loaded.
      m_PortalCameras.Add(
          currentCamera,
          portalCamera); // add the newly created camera to the dictionary
    }
  }
  ...
  private void CopyCameraProperties(Camera src, Camera dest) {
    ...
    // set camera to clear the same way as current camera <<< Not really sure
    ...
    dest.stereoTargetEye =
        StereoTargetEyeMask
            .None; // To prevent the camera from following some eye, else this
                   // gets fuckey sometimes (e.g. the FOV cant be copied)
  ...
  private Vector4 CameraSpacePlane(Matrix4x4 worldToCameraMatrix, Vector3 pos,
...
#region HelperMethods
  ...
  private static void MakeProjectionMatrixOblique(ref Matrix4x4 matrix,
                                                  Vector4 clipPlane) {
    Vector4 q;
