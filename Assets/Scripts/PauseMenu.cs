...
public class PauseMenu : MonoBehaviour {
  ...
  [SerializeField]
  private Sprite[] deactivatedLanguageSprites = new Sprite[2];
  [SerializeField]
  private Sprite[] activatedLanguageSprites = new Sprite[2];
  [SerializeField]
  private Image[] languageDisplays = new Image[2];

  [Header("Text animations")]
  [SerializeField]
  private Color focusTextColor = Color.white;
  [SerializeField]
  private Color unfocusedTextColor = Color.grey;
  [SerializeField]
  private TMP_Text resumeButtonText;
  [SerializeField]
  private TMP_Text language1Text;
  [SerializeField]
  private TMP_Text language2Text;
  ...
  public void Resume() { pauseCanvas.SetActive(false); }

  private void InitLanguageDisplay() { ToggleLocaleDisplay(isEnglishSelected); }

  public void ToggleLanguage() {
    string targetLanguage =
        LocalizationSettings.SelectedLocale.ToString() == "English (en)" ? "fr"
                                                                         : "en";
    LocalizationSettings.SelectedLocale =
        LocalizationSettings.AvailableLocales.GetLocale(targetLanguage);
  ...
  void ToggleLocaleDisplay(bool isEnglish) {
    ...
    languageDisplays[0].sprite =
        isEnglish ? activatedLanguageSprites[0] : deactivatedLanguageSprites[0];
    languageDisplays[1].sprite =
        isEnglish ? deactivatedLanguageSprites[1] : activatedLanguageSprites[1];
